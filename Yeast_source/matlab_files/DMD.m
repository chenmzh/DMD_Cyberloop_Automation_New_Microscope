...
classdef DMD < handle
    ...
    methods
        ...
        function definePattern(dmd) % 0x1A34
            ...
            idx             = 0;    % pattern index
%             exposureTime    = 1800*1e4;  % exposure time in seconds (first value)
            exposureTime    = 10*1e4;  % exposure time in seconds (first value)
        ...
        function uploadPattern(dmd, BMP) % 0x1A2B
            ...
            % Only show progress bar in desktop mode
            show_progress = usejava('desktop');
            if show_progress
            ...
            for i = 1:numOfTransfers
                ...
                while ~transmission_success && retry_count < max_retries
                    try
                        dmd.send(cmd);
                        rmsg = dmd.receive;
                        if cmd.msg(2) == rmsg(2)
                            ...
                            if retry_count < max_retries
                                pause(0.1); % Wait 100ms before retry
                            end
                        end
                    catch ME
                        retry_count = retry_count + 1;
                ...
                if ~transmission_success
                    % Just a small patch to fix the error for Nicolo and Mingzhe Yeast experiment
                    error('Transmission error: sent message 0x%02X, received 0x%02X after %d retries', ...
                          cmd.msg(2), rmsg(2), max_retries);
                end
